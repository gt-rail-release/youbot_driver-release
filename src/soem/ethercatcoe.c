            ...
            if (wkc > 0) /* succeeded to place mailbox in slave ? */
            {
              ...
              if (wkc > 0) /* succeeded to read slave response ? */
              {
                ...
                if (((aSDOp->MbxHeader.mbxtype & 0x0f) == ECT_MBXT_COE)
                    && ((etohs(aSDOp->CANOpen) >> 12) == ECT_COES_SDORES) && (aSDOp->Index == SDOp->Index))
                {
                    ...
                    && (aSDOp->SubIndex == SDOp->SubIndex))
                {
                  ...
                  while (NotLast)
                  {
                    ...
                    if (wkc > 0)
                    {
                      ...
                      {
                        if (((aSDOp->MbxHeader.mbxtype & 0x0f) == ECT_MBXT_COE)
                            && ((etohs(aSDOp->CANOpen) >> 12) == ECT_COES_SDORES) && ((aSDOp->Command & 0xe0) == 0x20))
                        {
...
// end slave bug prevention code
          ...
          int ec_readODdescription(uint16 Item, ec_ODlistt *pODlist)
          {
            ...
            if (wkc > 0)
            {
              ...
              if (wkc > 0)
              {
                if (((aSDOp->MbxHeader.mbxtype & 0x0f) == ECT_MBXT_COE) && ((aSDOp->Opcode & 0x7f) == ECT_GET_OD_RES))
                {
          ...
          static int ec_readOEsingle(uint16 Item, uint8 SubI, ec_ODlistt *pODlist, ec_OElistt *pOElist)
          {
            ...
            if (wkc > 0)
            {
              ...
              if (wkc > 0)
              {
                if (((aSDOp->MbxHeader.mbxtype & 0x0f) == ECT_MBXT_COE) && ((aSDOp->Opcode & 0x7f) == ECT_GET_OE_RES))
                {
                  pOElist->Entries++;
