...
int ec_findconfig(uint32 man, uint32 id)
{
  ...
  {
    i++;
  } while ((ec_configlist[i].man != EC_CONFIGEND) && ((ec_configlist[i].man != man) || (ec_configlist[i].id != id)));
    ...
    for (slave = 1; slave <= ec_slavecount; slave++)
    {
      ...
      if (slave > 1)
      {
        ...
        do
        {
          ...
          if (topology == 4)
            topoc += 2; /* cross found */
          if (((topoc >= 0) && (topology > 1)) || (slavec == 1)) /* parent found */
          {
      ...
      else
      {
        ...
        {
          sprintf(ec_slave[slave].name, "? M:%8.8x I:%8.8x", (unsigned int)ec_slave[slave].eep_man,
