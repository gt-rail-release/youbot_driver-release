...
typedef struct
    ...
    if (wkc > 0) /* succeeded to place mailbox in slave ? */
    {
      while (NotLast)
      {
        ...
        if (wkc > 0) /* succeeded to read slave response ? */
        {
          ...
          if (((aSoEp->MbxHeader.mbxtype & 0x0f) == ECT_MBXT_SOE) && (aSoEp->opCode == ECT_SOE_READRES)
              && (aSoEp->error == 0) && (aSoEp->driveNo == driveNo) && (aSoEp->elementflags == elementflags))
          {
      ...
      if (wkc > 0) /* succeeded to place mailbox in slave ? */
      {
        if (!NotLast || !ec_mbxempty(slave, timeout))
        {
          ...
          if (wkc > 0) /* succeeded to read slave response ? */
          {
            ...
            if (((aSoEp->MbxHeader.mbxtype & 0x0f) == ECT_MBXT_SOE) && (aSoEp->opCode == ECT_SOE_WRITERES)
                && (aSoEp->error == 0) && (aSoEp->driveNo == driveNo) && (aSoEp->elementflags == elementflags))
            {
              /* SoE write succeeded */
            }
  ...
  int ec_readIDNmap(uint16 slave, int *Osize, int *Isize)
  {
    ...
    *Osize = 0;
    ...
    if ((wkc > 0) && (psize >= 4) && ((entries = etohs(SoEmapping.currentlength) / 2) > 0)
        && (entries <= EC_SOE_MAXMAPPING))
    {
